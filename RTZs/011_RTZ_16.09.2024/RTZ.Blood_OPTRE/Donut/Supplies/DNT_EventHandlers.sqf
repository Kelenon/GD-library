/* DNT_onReloadEH
*	Clientside EH responsible for depleting ammo on player reloading an empty or almost empty mag 
*	Params:
*		_playerUnit - player to which EH is to be attached to
*/
...
DNT_onReloadEH = {
	...
	_playerUnit addEventHandler ["Reloaded", {
		...
		if(_ammoLeftPercent<75) then {
			remoteExec ["DNT_onReloadServer",2];
		};
	}];
};

/* DNT_onTreatmentEH
*	Clientside EH responsible for depleting meds on player performing medical 
*/
...
DNT_onTreatmentEH = {
	["ace_treatmentSucceded", {
		...
		if(isNil "dnt_medicalItemsUsed") then {
		
			dnt_medicalItemsUsed = 0;
			publicVariableServer "dnt_medicalItemsUsed";
		
		};
		
		if(_usedItem != "") then {
			
			dnt_medicalItemsUsed = dnt_medicalItemsUsed + 1; 
			publicVariableServer "dnt_medicalItemsUsed";
			
		};	
		
		if(dnt_medicalItemsUsed >=8) then {
		
			remoteExec ["DNT_onTreatmentServer", 2];
			dnt_medicalItemsUsed = 0;
			publicVariableServer "dnt_medicalItemsUsed";
		};
...
/* DNT_fortifySpendScrap
*	Clientside EH responsible for depleting scrap on player bulding fortification
*	
*
*/
...
DNT_fortifySpendScrap = {
  ...
  ["ace_fortify_deployFinished", { 
    _this select 0 params ["_player", "_side", "_configName", "_posASL", "_vectorDir", "_vectorUp","_cost"];
    
    [_cost] remoteExec ["DNT_onFortifyServer",2];
...
/* DNT_fortifyDeployHandler
* 	EH to limit when placting a fortification is possible
*/
...
DNT_fortifyDeployHandler = {
	...
	[{
	    ...
	    private _isEnoughScrap = (dnt_scrapCount >= _cost);
	    private _return = _isEngineer && _isEnoughScrap;
	    _return
...
/* DNT_fortifyDeployHandler
* 	EH to recover some scrap on object deletion
*

DNT_fortifyObjectDeleted = {
  ...
  ["acex_fortify_objectDeleted", { 
    ...
    switch (true) do {
    	...
    	case (_objectDeleted in ["3AS_Cover1"]): {_value = 2};
		case (_objectDeleted in ["3AS_Barricade_2_C_Prop"]): {_value = 4};
		case (_objectDeleted in ["Land_lsb_fob_hBarrier_wall", "Land_lsb_fob_hBarrier_3", "Land_lsb_fob_hBarrierWall_ramp"]): {_value = 5};
		case (_objectDeleted in ["Land_lsb_fob_hBarrierWall_4", "Land_lsb_fob_hBarrierWall_exvertedCorner"]): {_value = 10};
		case (_objectDeleted in ["Land_lsb_fob_hBarrierWall_7"]): {_value = 15};
		case (_objectDeleted in ["Land_lsb_fob_hBarrier_tower"]): {_value = 50};
		case (_objectDeleted in ["3AS_Pad_GARR3_Prop"]): {_value = 200};
		
    };
   
   ["scrap",_value] call DNT_clientWriteToDb;
...
/* DNT_onResDepletionChange
*	Function launches when resources are either depleted or replenished to limit access to certain pieces of equipment. 
*	
*
*/
DNT_onResDepletionChange = {
		...
		{	
			if(isNull (getAssignedCuratorLogic player)) then {
				...
				if(dnt_ammoDepleted == true) then {
					...
					[ace_arsenal_currentBox, _standardWeapons] call ace_arsenal_fnc_removeVirtualItems;

				} else {
							
					[ace_arsenal_currentBox, _standardWeapons] call ace_arsenal_fnc_addVirtualItems;
				...
				if(dnt_medsDepleted == true) then {
					...
					[ace_arsenal_currentBox, _standardMeds] call ace_arsenal_fnc_removeVirtualItems;
						
				} else {
					
					[ace_arsenal_currentBox, _standardMeds] call ace_arsenal_fnc_addVirtualItems;
					
				};
				
				[ace_arsenal_fnc_refresh, [true,true,true], 5] call CBA_fnc_execAfterNFrames;
...
/* DNT_onSupplyLost
* 	EH to handle destroyed supplies or supplies removed by zeus
*/
