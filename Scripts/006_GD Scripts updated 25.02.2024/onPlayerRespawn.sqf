...
if(dnt_ammoCount>0 && dnt_medicalCount>0) then {player setUnitLoadout(player getVariable["Saved_Loadout",[]]);}

else{

	if(dnt_ammoCount <= 0 && dnt_medicalCount <= 0) then {
		player setUnitLoadout(player getVariable["Saved_Loadout",[]]);
		{player removeWeapon _x;}forEach dnt_standardWeapons;
		{player removeItems _x;}forEach dnt_standardWeapons;
		{player removeMagazine _x} forEach magazines player;
		{player removeItems _x;}forEach dnt_standardMeds;
...
player addEventHandler ["Reloaded", {
	...
	if(_ammoLeftPercent<=10) then {
		if(dnt_ammoCount>0) then {dnt_ammoCount = dnt_ammoCount - 1; publicVariable "dnt_ammoCount";}
		else{dnt_ammoCount = 0;};
...
["ace_treatmentStarted", {
	...
	if(dnt_medicalItemsUsed >=10) then {
		if(dnt_medicalCount >0) then{dnt_medicalCount = dnt_medicalCount - 1;publicVariable "dnt_medicalCount";}
		else {dnt_medicalCount = 0;publicVariable "dnt_medicalCount"};
